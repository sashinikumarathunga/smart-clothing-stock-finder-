  ...
  render: async (container, flash) => {
    const load = async () => {
      ...
      container.querySelectorAll('.deactivate-btn').forEach((btn) => {
        btn.onclick = async () => {
          if (!confirm('Deactivate this branch? The record will remain in the database but disappear from the system.')) return;
          await apiDelete(`/api/branches/${btn.dataset.id}`);
          showFlash(flash, 'success', 'Branch deactivated');
          await load();
        };
      });
