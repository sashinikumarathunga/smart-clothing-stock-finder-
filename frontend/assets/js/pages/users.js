  ...
  render: async (container, flash, user) => {
    ...
    const load = async () => {
      ...
      container.querySelectorAll('.deactivate-btn').forEach((btn) => {
        btn.onclick = async () => {
          if (!confirm('Deactivate this user? The account will remain in the database and will no longer be able to log in.')) return;
          await apiDelete(`/api/users/${btn.dataset.id}`);
          showFlash(flash, 'success', 'User deactivated');
          await load();
        };
      });
