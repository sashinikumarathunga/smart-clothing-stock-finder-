  ...
  render: async (container, flash) => {
    const load = async () => {
          ...
          [
            ...
            { key: 'email', label: 'Email' },
            {
              ...
              render: (r) => `<button class="btn btn-sm btn-outline-primary me-1 edit-btn" data-id="${r.id}">Edit</button>
                <button class="btn btn-sm btn-outline-warning deactivate-btn" data-id="${r.id}"><i class="bi bi-slash-circle me-1"></i>Deactivate</button>`,
            },
          ],
          suppliers
        )}</div>
      ...
      container.querySelectorAll('.deactivate-btn').forEach((btn) => {
        btn.onclick = async () => {
          if (!confirm('Deactivate this supplier? The record will remain in the database but disappear from the system.')) return;
          await apiDelete(`/api/suppliers/${btn.dataset.id}`);
          showFlash(flash, 'success', 'Supplier deactivated');
          await load();
        };
      });
